...
interface EditaisClientProps {
    ...
    initialEditalTypes: FilterOption[];
...
export default function EditaisClient({ 
 ...
 const [editalTypes, setEditalTypes] = useState<FilterOption[]>(initialEditalTypes);
 const [campi, setCampi] = useState<FilterOption[]>(initialCampi);

    
 const typesFromUrl = searchParams.get('tipos')?.split(',').filter(Boolean) || [];
 ...
 async function fetchData() {
        ...
        const currentTypes = searchParams.get('tipos')?.split(',').filter(Boolean) || [];
        const currentCampi = searchParams.get('campi')?.split(',').filter(Boolean) || [];
        const currentPage = Number(searchParams.get('page')) || 1;

 const filterConditions = [
 currentTypes.length > 0 ? `editalType._ref in [${currentTypes.map(id => `'${id}'`).join(',')}]` : '',
 currentCampi.length > 0 ? `campus._ref in [${currentCampi.map(id => `'${id}'`).join(',')}]` : '',
 ].filter(Boolean).join(' && ');
...
`;
 ...
 const [fetchedEditais, fetchedTotalItems] = await Promise.all([
